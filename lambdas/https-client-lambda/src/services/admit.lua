...
local cbEnabled        = tonumber(ARGV[8]) == 1
...
local cbNeedInit       = state[1] == false or state[1] == nil
local rlNeedInit       = state[4] == false or state[4] == nil
local isOpen           = cbNeedInit or tonumber(state[1]) == 1
local switchedAt       = cbNeedInit and 0 or tonumber(state[2] or "0")
local bucketTokens     = tonumber(state[3] or "0")
local bucketRefilledAt = rlNeedInit and now or tonumber(state[4])
