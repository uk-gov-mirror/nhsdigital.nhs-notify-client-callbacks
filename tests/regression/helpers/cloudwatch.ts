...
export async function queryCallbacksFromWebhookLogGroup(
  client: CloudWatchLogsClient,
  logGroupName: string,
  messageId: string,
  callbackType: CallbackItem["type"],
  ...
  return querySignedCallbacksFromWebhookLogGroup(
    client,
    logGroupName,
    messageId,
    callbackType,
...
type TransformLogEntry = Record<string, unknown>;

export async function queryTransformLambdaLogs(
  client: CloudWatchLogsClient,
  logGroupName: string,
  messageId: string,
  startTime: number,
): Promise<TransformLogEntry[]> {
  const queryStartTime = Math.max(0, startTime - CLOUDWATCH_QUERY_LOOKBACK_MS);
  const filterPattern = `{ $.messageId = "${messageId}" }`;
  ...
  const entries: TransformLogEntry[] = [];
  for (const event of response.events ?? []) {
    if (event.message) {
      try {
        entries.push(JSON.parse(event.message) as TransformLogEntry);
      } catch {
        // skip unparseable entries
      }
    }
