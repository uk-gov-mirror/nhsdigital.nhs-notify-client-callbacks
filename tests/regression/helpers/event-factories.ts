...
import { getRegressionClientConfig } from "./mock-client-config";
...
export function createMessageStatusPublishEvent(
  ...
  const baseData: MessageStatusData = {
    ...
    messageStatusDescription: "Regression test message delivered",
    channels: [
      {
        type: "NHSAPP",
...
export function createChannelStatusPublishEvent(
  ...
  const baseData: ChannelStatusData = {
    ...
    channel: "NHSAPP",
    channelStatus: "DELIVERED",
    channelStatusDescription: "Regression test channel delivered",
